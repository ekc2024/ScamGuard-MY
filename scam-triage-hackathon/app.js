...
import { isEnrichableCategory } from "./lib/qwen-enrich.mjs";
...
/**
 * Optional enrichment: a plain-language paragraph from Qwen, requested only for
 * the three fully-covered categories and only when the user opts in. The
 * verdict above is already final - this never changes it, and any failure just
 * leaves the verdict as it is.
 */
async function enrich(result, message, before) {
  if (!enrichToggle.checked || !isEnrichableCategory(result.category)) return;

  const section = element("section", "step enrichment");
  section.append(element("h3", null, "AI plain-language explanation (optional, does not change the verdict)"));
  const paragraph = element("p", null, "Asking Qwen for a plain-language explanation...");
  section.append(paragraph);
  resultBox.insertBefore(section, before);
  ...
  try {
    ...
    enrichment = response.ok ? (await response.json()).enrichment : null;
  } catch {
    enrichment = null;
  }

  if (enrichment?.available) {
    paragraph.textContent = enrichment.explanation;
    section.append(element("p", "hint", `Generated by ${enrichment.model}. The verdict above is unchanged.`));
    return;
  }

  paragraph.textContent =
    enrichment?.reason ?? "The optional explanation service is unavailable. The verdict above is unaffected.";
}
...
async function main() {
  ...
  form.addEventListener("submit", (event) => {
    ...
    const disclaimerBox = renderResult(result);
    void enrich(result, message, disclaimerBox);
  });
}
