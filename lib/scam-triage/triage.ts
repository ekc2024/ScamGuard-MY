import { CATEGORY_BY_ID, CATEGORY_DEFINITIONS } from "./categories";
import { checkReferenceList } from "./check-reference-list";
import { checkUrl } from "./check-url";
import { DISCLAIMERS } from "./disclaimers";
import { extractIdentifiers } from "./extract";
import type { CategoryScore, ReferenceMatch, TriageResult, UrlCheck, Verdict } from "./types";
...
function reportChannelsFor(top: CategoryScore | null, extracted: ReturnType<typeof extractIdentifiers>) {
  const channels: { channel: string; detail: string }[] = [REPORT_CHANNELS.nsrc];
...
export function triageMessage(rawText: string): TriageResult {
  ...
  return {
    verdict,
    headline,
    category: confident && top ? top.category : null,
    categoryLabel: confident && top ? top.label : null,
    coverage: confident && top ? top.coverage : null,
    confidence: top?.confidence ?? "low",
    stop: confident && definition ? definition.stop : GENERIC_STOP,
    check: {
      urls: urlChecks,
      references,
      userActions: confident && definition ? definition.userActions : GENERIC_ACTIONS,
    },
